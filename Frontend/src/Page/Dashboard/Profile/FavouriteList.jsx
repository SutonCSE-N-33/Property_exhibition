...
const FavouriteList = () => {
  const { isLoaded, favouriteData } = useFavourite();
  ...
  const favouriteItems = properties?.filter(property =>
    userWiseFavourite.some(favourite => favourite?.property === property?.id)
  );
        ...
        <div className="relative overflow-x-auto shadow-md md:rounded-lg p-5">
          <table className="md:w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {!isLoaded ? (
                <>
                  {favouriteItems.map((favourite, id) => (
                    <tr
                      key={id}
                    ...
                    >
                      ...
                      <td className="px-1.5 py-1.5"> {favourite.area} </td>
                      <td className="px-1.5 py-1.5"> {favourite.price} </td>
                      <td className="px-1.5 py-1.5 "> {favourite.type} </td>
                      <td className="px-1.5 py-1.5 "> {favourite.purpose} </td>
                      <td className="px-1.5 py-1.5 ">
                        {" "}
                        {favourite.completion}{" "}
                      </td>
                      <td className="px-1.5 py-1.5"> {favourite.division} </td>
                      <td className="flex items-center px-2 py-2">
                        <Link
                          to={`/property/${favourite.id}`}
